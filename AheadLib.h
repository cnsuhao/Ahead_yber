...
public:
	...
	static TCHAR m_tzPROJECTNAME[MAX_PATH];
	static TCHAR m_tzAPIDbgDSWPATH[MAX_PATH];
	static TCHAR m_PRJNAME[MAX_PATH];
...
public:
	// 写入到 文件
    static BOOL WINAPI WriteSzFile(PCSTR pszBuffer, DWORD dwSize,const char * szPath);
	static BOOL WINAPI WriteApiDebugger_CPP();
	// 生成
	static ALRESULT WINAPI Generate(BOOL bWriteCppFile = FALSE,BOOL bdirCustomer=FALSE);
	static BOOL WINAPI GenDSW();
	static BOOL WINAPI GenRc();
	
    static BOOL WINAPI GenRcDswApiDbg(const char * cppApiDbgPath);
	// 设置选项可用状态
	...
	// 获取结果文本
	inline static LPCSTR WINAPI GetResult(ALRESULT alResult)
	{
...
private:
    ...
    //生成hook mem 代码  by bob
	static DWORD WINAPI GenerateMemHookCode(PSTR pszBuffer, PSTR* ppszNames, DWORD dwBase, DWORD dwCount, BOOL& bLoadOrigin);
	static DWORD WINAPI GenerateMemHookCPPFullCode(PSTR pszBuffer, PSTR* ppszNames, DWORD dwBase, DWORD dwCount, BOOL& bLoadOrigin);
	// 生成导出指示符
    ...
    static VOID WINAPI GenerateMemHookFunction(PSTR& pszBuffer, PSTR pszName, DWORD dwBase, DWORD dwIndex);
	// 生成合法的函数名称
	static VOID WINAPI GenerateDeclaration(PSTR pszDeclaration, PSTR pszCallName, PSTR pszName, DWORD Ordinals);
    static VOID WINAPI GenerateMemHookDeclaration(PSTR pszDeclaration, PSTR pszCallName, PSTR pszName, DWORD Ordinals);
	// 把 RVA 转换为文件偏移
	...
	// 写入到 CPP 文件
	static BOOL WINAPI WriteCppFile(PCSTR pszBuffer, DWORD dwSize,BOOL bdirCustomer=FALSE);
